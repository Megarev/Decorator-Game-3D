...
private:
	void RasterizeMesh(const Mesh& mesh, std::vector<Triangle>& raster) {
		...
		for (const auto& v : mesh.t) {
			//Matrix4x4 rotated_world = Matrix4x4::MatrixMultiply(world, Matrix4x4::GetRotationMatrixY(mesh.rotation));
			...
			if (normal.dot(translated.v[0] - cam_pos) < 0.0f) {
				...
				float p = std::fmaxf(0.1f, (normal.dot(light_dir.norm())));
...
public:
	...
	void Input(olc::PixelGameEngine* pge, float dt, float move_speed = 8.0f, float rotation_sensitivity = 2.0f) {
		...
		if (pge->GetKey(olc::A).bHeld) yaw -= rotation_sensitivity * dt;
		else if (pge->GetKey(olc::D).bHeld) yaw += rotation_sensitivity * dt;
	}
	...
	void DrawTexturedTriangle(olc::PixelGameEngine* pge, SpriteData s1, SpriteData s2, SpriteData s3, olc::Sprite* sprite = nullptr) {
		...
		/*move_step1.tex.u = Clamp(offset1.tex.u / sprite->height, 0.0f, 1.0f);
		move_step1.tex.v = Clamp(offset1.tex.v / sprite->height, 0.0f, 1.0f);

		move_step2.tex.u = Clamp(offset2.tex.u / sprite->height, 0.0f, 1.0f);
		move_step2.tex.v = Clamp(offset2.tex.v / sprite->height, 0.0f, 1.0f);*/
	...
	void DrawCube(olc::PixelGameEngine* pge, const vf3d& position, const olc::Pixel& color, bool is_wire_frame = false, const olc::Pixel& wire_frame_color = olc::WHITE) {
		DrawCuboid(pge, position, vf3d::ONE, color, is_wire_frame, wire_frame_color);
	}

	void DrawCuboid(olc::PixelGameEngine* pge, const vf3d& position, const vf3d& scale, const olc::Pixel& color, bool is_wire_frame = false, const olc::Pixel& wire_frame_color = olc::WHITE) {
		Mesh cube;
